  ...
  addKeyboardShortcuts() {
    return {
      Enter: () => {
        ...
        const match = text.match(/(?:^|\s)\/log\s+(.+)$/i);

        if (match) {
          const rawText = match[1].trim();
          const matchIndex = match.index || 0;
          
          // Adjust starting position to preserve text before the space (if any)
          const matchedStr = match[0];
          const hasLeadingSpace = matchedStr.startsWith(' ') || matchedStr.startsWith('\t');
          const fromPos = $from.start() + matchIndex + (hasLeadingSpace ? 1 : 0);
          const toPos = $from.start() + matchIndex + matchedStr.length;
  ...
  addKeyboardShortcuts() {
    return {
      Enter: () => {
        ...
        const text = $from.parent.textContent;
        const match = text.match(/(?:^|\s)\/(remind|alert)\s+(.+)$/i);
