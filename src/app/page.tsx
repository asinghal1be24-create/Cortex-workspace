...
export default function Home() {
  ...
  const handleTemplateSelect = (selectedFiles: WorkspaceFile[]) => {
    setFiles(prev => {
      ...
      selectedFiles.forEach(newFile => {
        if (!merged.some(f => f.name.toLowerCase() === newFile.name.toLowerCase())) {
          merged.push(newFile);
        }
      });
      
      const active = selectedFiles[0];
      const matched = merged.find(f => f.name.toLowerCase() === active.name.toLowerCase());
      if (matched) {
        setActiveFileId(matched.id);
      }
      
      saveToStorage(merged, pagesMap);
      return merged;
    });
    setFocusMode(false);
  };

  // Sync / Fetch files from Supabase when user logs in
  useEffect(() => {
    if (authLoading) return;
    
    async function syncFilesOnLogin() {
      if (user) {
        try {
          const res = await fetch("/api/files");
          if (!res.ok) {
            console.error("[Cortex Sync] Failed to fetch files from API route:", res.statusText);
            return;
          }
          const data = await res.json();

          console.log("[Cortex Sync] Fetched raw database data:", data);

          const dbFiles: WorkspaceFile[] = [];
          const newPagesMap: PagesMap = {};

          (data || []).forEach((row: any) => {
            let content = row.content ?? "";
            
            // Check if content contains serialized page data
            if (content.trim().startsWith("[")) {
              try {
                const parsed = JSON.parse(content);
                if (Array.isArray(parsed) && parsed.every(p => p && typeof p === 'object' && 'id' in p)) {
                  newPagesMap[row.id] = parsed;
                  // Fallback content value is the content of the first page
          ...
          console.log("[Cortex Sync] Decoded files list:", dbFiles);
          console.log("[Cortex Sync] Decoded pages map:", newPagesMap);

          // Update pagesMap state and cache safely (local-first)
          setPagesMap(prev => {
            const mergedPages = { ...newPagesMap };
            Object.keys(prev).forEach(fileId => {
              const localPages = prev[fileId];
              const dbPages = mergedPages[fileId];
              
              if (dbPages) {
                const localSerialized = JSON.stringify(localPages);
                const dbSerialized = JSON.stringify(dbPages);
                // If local pages contain unsaved text/sketch edits, preserve them
                if (localSerialized !== dbSerialized && localPages.some(p => p.content.trim() !== "")) {
                  mergedPages[fileId] = localPages;
                }
              } else {
                mergedPages[fileId] = localPages;
              }
            });
            localStorage.setItem(PAGES_KEY, JSON.stringify(mergedPages));
            return mergedPages;
          });

          // Merge: add files from local state, resolving conflicts in favor of local unsaved content
          setFiles(prev => {
            let merged = [...dbFiles];
            prev.forEach(localFile => {
              const dbMatch = merged.find(dbFile => dbFile.id === localFile.id || dbFile.name === localFile.name);
              if (dbMatch) {
                // If the local file has different, non-empty content, prioritize the local unsaved work
                if (localFile.content !== dbMatch.content && localFile.content.trim() !== "") {
                  dbMatch.content = localFile.content;
                }
              } else {
          ...
          console.error("Failed to merge files:", e);
        }
      }
    }
  ...
  const handleSave = async () => {
    ...
    if (user) {
      try {
        const rowsToUpsert = files.map(f => {
        ...
        console.log("[Cortex Sync] Preparing to sync files to API route. Payload:", rowsToUpsert);
        ...
        if (!res.ok) {
          console.error("[Cortex Sync] API sync failed:", res.statusText);
        } else {
          console.log("[Cortex Sync] Successfully synced all files to cloud database!");
          setDeletedFileIds([]);
        }
      } catch (e) {
        console.error("Failed to sync to API route:", e);
      }
    }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Logo */}
        <div style={{ padding: '0px', borderBottom: `1px solid ${B.border}`, display: 'flex', justifyContent: 'center', alignItems: 'center', height: '64px', overflow: 'hidden' }}>
          <div 
            onClick={() => { setActiveFileId(null); if (isMobile) setMobileMenuOpen(false); }} 
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flex: 1, height: '100%', width: '100%', overflow: 'hidden' }}
          >
            <img src="/logo.jpeg" alt="Cortex" style={{ height: '85%', width: '85%', objectFit: 'contain', objectPosition: 'center', mixBlendMode: 'screen' }} />
          </div>
          {isMobile && (
            <button onClick={() => setMobileMenuOpen(false)} style={{ background: 'none', border: 'none', color: B.muted, fontSize: 24, cursor: 'pointer', paddingLeft: 12 }}>×</button>
          )}
        </div>

        <div id="sidebar-toggles" style={{ padding: '10px 10px 0', borderBottom: `1px solid ${B.border}`, paddingBottom: 10 }}>
          <div style={{
        ...
        {/* Files list */}
        {section === 'files' && (
          <div id="workspace-files" style={{ flex: 1, overflowY: 'auto', padding: '12px 8px' }}>
            <div style={{
        ...
        <div style={{ padding: '12px', borderTop: `1px solid ${B.border}` }}>
          ...
          {/* Help & Tour button */}
          <button 
            onClick={() => window.dispatchEvent(new Event('restart-cortex-tour'))}
            style={{
              width: '100%', marginTop: '8px', padding: '6px 12px', borderRadius: 6, fontSize: 11,
              color: B.muted, border: 'none', background: 'transparent',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
